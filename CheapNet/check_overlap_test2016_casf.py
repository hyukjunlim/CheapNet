...
csar_values = csar_df['pdbid'].values
for i, value in enumerate(csar_values):
    if value in test2016_df['pdbid'].values:
        print(i, value, 'in test2016_df')
    if value in test2019_df['pdbid'].values:
        print(i, value, 'in test2019_df')
    if value in casf2016_df['pdbid'].values:
        print(i, value, 'in casf2016_df')
    if value in train_df['pdbid'].values:
        print(i, value, 'in train_df')
    if value in valid_df['pdbid'].values:
        print(i, value, 'in valid_df')
    if value in test2013_df['pdbid'].values:
        print(i, value, 'in test2013_df')
# # check cid '1hvr' in valid_df, test2013_df, test2016_df, test2019_df, csar_df, casf2016_df
# print('train_df:', '1hvr' in train_df['pdbid'].values)
